...
such as `['a', 'b', 'c']`.
The term [list](#list) is often used synonymously.
...
Functional programming in JavaScript relies heavily on [callbacks](#callback-function)
and [higher-order functions](#higher-order-function).

**global intallation**{:#global-intallation}:
FIXME
...
in [arrays](#array) and name/value structures.
...
from [arguments](#argument) (the values passed in),
...
(Some languages call this *[raising](#raise)* an exception.)
...
Every node except the [root-node](#root-node) must have exactly one [parent node](#parent-node),
