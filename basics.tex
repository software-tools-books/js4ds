...
\gref{g:string}{Character strings} like \texttt{'hello,\ world'}
...
JavaScript's \texttt{boolean} type\index{datatype!Boolean}
...
We start with arrays that contain other arrays,\index{nesting!arrays}\index{array!nested}
which are usually processed by \gref{g:nested-loop}{nested loops}:\index{loop!nested}
...
we can \grefdex{g:string-interpolation}{interpolate}{string interpolation}
...
The keys do not have to be strings,\index{object!keys}
...
its name,
a parameterized list of \grefdex{g:parameter}{parameters}{parameter}\index{function!parameter}
...
each one is a \grefdex{g:scope}{scope}{scope!variable},\index{variable scope}
